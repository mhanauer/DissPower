...
```
Appendix Rcode 4a: Power analysis for EFA .7
```{r}
set.seed(123)
...
power_efa_agg
write.csv(power_efa_agg,"power_efa_agg_7.csv", row.names = FALSE)
```
Appendix Rcode 4b: Power analysis for EFA .6
```{r}
set.seed(123)
n_sample = seq(from = 80, to = 140, by = 10)
...
fa_vss = list()
for(i in 1:length(n_sample)){
dat_vss[[i]] = sim_decile(ncases=n_sample[[i]], nvariables=10, nfactors=1, meanloading=.6)
fa_vss[[i]] = fa(dat_vss[[i]], 1, rotate="varimax", cor = "cor")
tli_out[[i]] = fa_vss[[i]]$TLI
rmsea[[i]] = fa_vss[[i]]$RMSEA[1]
chi_squre_p[[i]] = fa_vss[[i]]$PVAL 
...
power_efa_agg[,2:4] =  round(power_efa_agg[,2:4]/reps,3)
power_efa_agg
write.csv(power_efa_agg,"power_efa_agg_6.csv", row.names = FALSE)
```
Appendix Rcode 4c: Power analysis for EFA .5
```{r}
set.seed(123)
n_sample = seq(from = 80, to = 140, by = 10)
...
fa_vss = list()
for(i in 1:length(n_sample)){
dat_vss[[i]] = sim_decile(ncases=n_sample[[i]], nvariables=10, nfactors=1, meanloading=.5)
fa_vss[[i]] = fa(dat_vss[[i]], 1, rotate="varimax", cor = "cor")
tli_out[[i]] = fa_vss[[i]]$TLI
rmsea[[i]] = fa_vss[[i]]$RMSEA[1]
chi_squre_p[[i]] = fa_vss[[i]]$PVAL 
...
power_efa_agg[,2:4] =  round(power_efa_agg[,2:4]/reps,3)
power_efa_agg
write.csv(power_efa_agg,"power_efa_agg_5.csv", row.names = FALSE)
```
